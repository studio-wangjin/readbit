...
export function ArticleSectionPage({ article }: Props) {
  ...
  const handleSectionChange = useCallback(
    ...
    [router, searchParams, resetSelection]
  );
  ...
  useEffect(() => {
    ...
    if (sectionFromUrl >= 0 && sectionFromUrl < sections.length) {
      if (sectionFromUrl !== currentSectionIndex) {
        resetSelection();
      }
      setCurrentSectionIndex(sectionFromUrl);
    }
  }, [searchParams, sections.length, currentSectionIndex, resetSelection]);

  const currentSection = sections[currentSectionIndex];

  const handleSentenceSelect = (text: string, index: number) => {
  ...
  return (
    <div className="container mx-auto px-4 py-8 pb-[calc(2rem+12rem)] relative min-h-screen">
      ...
      <ArticleSectionNavigation
        ...
        note={selectedSentence}
        onNoteChange={setSelectedSentence}
